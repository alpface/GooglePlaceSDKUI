...
- (UIView *)xy_noDataPlacehodlerCustomViewWithUse:(BOOL *)use {
    UIView *view = nil;
    BOOL u = NO;
    ...
    if (self.xy_loading == NO && view == nil) {
        u = NO;
    }
    if (use) {
        *use = u;
...
- (void)xy_reloadNoData {
    ...
    if (([self xy_noDataPlacehodlerShouldDisplay] && ![self xy_itemCount]) ||
        ...
        BOOL useCutomView = NO;
        UIView *customView = [self xy_noDataPlacehodlerCustomViewWithUse:&useCutomView];
        if (useCutomView) {
